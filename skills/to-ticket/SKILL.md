...
Verify the supplied bodies against the selected type templates and accepted scope. For each Story, confirm that the supplied plan identifies one stakeholder outcome and a valid acceptance boundary, that acceptance-significant terms resolve to an explicit rule or precise reference, and that related Stories do not conflict in ownership or behavior. A broad verb, shared entity, screen, or module does not establish one outcome. If correcting the problem requires splitting, merging, or redefining a Story, return that branch to definition rather than redesigning it during publication. Preserve requirement coverage, accepted decisions, rationale, and verification requirements. Mechanical formatting and link resolution belong here; new requirements, changed boundaries, or substantive contract rewrites return to definition. Keep native relationships outside issue bodies and preserve concurrent human content.
