...
At each new session, an adopting repository must load `$workflow`, verify the installed version declared in `AGENTS.md`, and read project-specific facts before workflow-dependent action. If the active version is missing or mismatched, report the blocked portion and continue valid independent work. In a repository without adoption, individual skills may serve a user request, but they must not apply this lifecycle, mutate Project state, or edit `AGENTS.md` to opt in.
